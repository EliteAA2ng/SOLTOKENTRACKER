...
export function AutoConnectWallet({ onConnectingChange }: AutoConnectWalletProps) {
  ...
  const triggerAutoConnect = async () => {
    ...
    try {
      ...
      const previousWallet = sessionStorage.getItem('walletName'); // Use sessionStorage instead
  ...
  useEffect(() => {
    if (connected && wallet) {
      ...
      sessionStorage.setItem('walletName', wallet.adapter.name); // Use sessionStorage instead
    }
  }, [connected, wallet]);
